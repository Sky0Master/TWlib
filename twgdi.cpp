#include "include/twgdi.hpp"
using namespace Gdiplus;

LPCWSTR ConvertChartoWchar(const char* s)  //将字符串转为宽字符串
{
...
void RGBChange(int* a, int* b, int* c)      //RGB渐变 
{
    static int da = 1;
    static int db = 1;
    static int dc = 1;
...
)
{
    ...
    DrawLine(x,y,ed_x,ed_y,w,r,g,b,hwnd);
}


    cat::cat(HWND _hwnd,int _x, int _y) :hwnd(_hwnd),x(_x), y(_y) {
        r = 255; g = 255; b = 255; angle = 0; w = 1;
        auto_shade = 0;
    } //初始颜色为白色

    void cat::setWidth(int width)
    {
        if (w > 0) return;
        w = width;
    }
    
    void cat::autoShade(int state)
    {
        if(state==0||state==1) 
            auto_shade=state;
    }

    void cat::setColor(int r,int g,int b) {           //设置颜色RGB
    ...
    void cat::changeColor()            //颜色变换
    {   
        RGBChange(&r,&g,&b);
    }
    void cat::changeColor(void (*rgbchange)(int*,int*,int*))      //颜色变换重载：传入rgb变换函数
    {
        rgbchange(&r,&g,&b);
    }
    ...
    void cat::setAngle(int ang)
    {
        this->angle = ang % 360;
    }

    void cat::right(int ang)
    {
        angle = (angle + ang) % 360;
    }

    void cat::left(int ang)
    {
        angle = (angle - ang) % 360;
    }